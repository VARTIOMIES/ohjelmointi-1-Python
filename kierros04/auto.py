...
def fill(size_of_tank, requested_gas, already_in_tank_gas):
    """
    ...
    input.
    :param size_of_tank: float,
    :param requested_gas: float,
    :param already_in_tank_gas: float,
    :return: float,

    """
    # Teoreettinen bensan määrä
    gas_in_tank = already_in_tank_gas + requested_gas

    # Bensaa ei voi kuitenkaan olla enempää kuin mitä tankkiin mahtuu.
    # Jos bensaa on liikaa, bensanmäärä muutetaan tankin suuruiseksi.
    if gas_in_tank >= size_of_tank:
        gas_in_tank = size_of_tank

    return gas_in_tank


def drive(from_x, from_y, to_x, to_y, gas, consumption):
    """
    ...
    # Let's see how far user wants to drive
    travel = distance_calculation(from_x, to_x, from_y, to_y)

    # Lets see what is the distance that the car can drive with remaining gas
    p = max_distance(gas, consumption)

    if travel > p:
        delta_x = p/travel * (to_x - from_x)
        delta_y = p/travel * (to_y - from_y)
        to_x = delta_x + from_x
        to_y = delta_y + from_y
        gas = 0.0
    else:
        gas = gas - travel * consumption / 100.0

    return gas, to_x, to_y


def distance_calculation(x0, x1, y0, y1):
    """
...
def max_distance(gas, consumption):
    """
    Calculates maximum distance with remaining gas

    :param gas: float,
    :param consumption: float,
    :return: float,
    """
    distance = gas / (consumption / 100.0)
    return distance
