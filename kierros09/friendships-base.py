...
def print_command(network, namelist):
    ...
    for someone in sorted(network):
        print(someone)
        for friend in sorted(network[someone]):
            print("-", friend)
...
def common_friends_command(network, namelist):
    ...
    #           print(f"{name1} has no common friends with him/herself.")
    common_list = []
    for friend_name1 in network[name1]:
        if friend_name1 in network[name2]:
            common_list.append(friend_name1)

    if len(common_list) == 0:
        print(f"{name1} and {name2} have no common friends.")
        return

    for friend in sorted(common_list):
        print(friend)
